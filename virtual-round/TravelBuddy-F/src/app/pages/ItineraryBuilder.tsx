...
export function ItineraryBuilder() {
  ...
  const moveActivity = (stopIndex: number, fromIndex: number, toIndex: number) => {
    ...
    const [removed] = newStops[stopIndex].stopActivities.splice(fromIndex, 1);
    newStops[stopIndex].stopActivities.splice(toIndex, 0, removed);
  ...
  return (
    <DndProvider backend={HTML5Backend}>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="flex items-center gap-2 bg-gradient-to-r from-accent to-primary px-4 py-2 rounded-xl text-white hover:opacity-90 transition-all"
                >
                  <Sparkles className="w-5 h-5" />
                  AI Suggest
                </button>

                <button onClick={() => setShowCollab(true)} className="flex items-center gap-2 bg-gradient-to-r from-primary to-secondary px-6 py-2 rounded-xl text-white hover:opacity-90 transition-all">
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 space-y-6">
                  ...
                  <div key={stop.id}>
                    ...
                    <div className="mb-4">
                      {(stop.stopActivities || []).length > 0 ? (
                        (stop.stopActivities || []).map((sa, actIndex) => (
                          <DraggableActivity
                            key={sa.id}
                            stopActivity={sa}
                            index={actIndex}
                ...
                <button onClick={() => setShowAddDest(true)} className="w-full bg-gradient-to-r from-primary/20 to-secondary/20 border border-primary/30 rounded-xl py-4 hover:opacity-80 transition-all flex items-center justify-center gap-2">
                  <Plus className="w-5 h-5" />
                  Add Destination
                </button>
              </div>
              ...
              <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-bold">
                        {trip.stops.reduce((sum, s) => sum + (s.stopActivities || []).length, 0)}
                      </span>
                    </div>
                ...
                <GlassCard className="p-6 cursor-pointer hover:bg-white/10 transition-all" onClick={() => setShowAi(true)}>
                  <h3 className="font-bold mb-2 text-primary flex items-center gap-2">
                    <Sparkles className="w-5 h-5" />
                    AI Insights
                  </h3>
                  <p className="text-sm text-muted-foreground">Click to generate a Gemini AI itinerary with day-by-day plans and pro tips!</p>
                </GlassCard>
              </div>
            </div>
          </div>
      ...
      {showAddDest && id && <AddDestinationModal tripId={id} onClose={() => setShowAddDest(false)} onAdded={fetchTrip} />}
      {showAi && trip && <AiSuggestPanel trip={trip} onClose={() => setShowAi(false)} />}
      {showCollab && trip && <CollaborationModal tripId={trip.id} tripName={trip.name} onClose={() => setShowCollab(false)} />}
    </DndProvider>
  );
}
